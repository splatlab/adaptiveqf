...
int main(int argc, char **argv) {
	...
	//clock_t query_start_clock, query_end_clock;
	...
	for (i = 0; i <= num_inserts; i++) {
		...
		if (i >= measure_point) {
			...
			for (size_t j = 0; j < num_inc_queries; j++) {
				if (filter.Contain(inc_queries[j]) == cuckoofilter::Ok) {
	...
	for (i = 0; i < num_queries; i++) {
		...
		if (i >= measure_point) {
			...
			if (i >= warmup_queries) {
				...
				fprintf(fprates_fp, "%lu %f\n", i - warmup_queries, (double)fp_queries / (i - warmup_queries));
			}
