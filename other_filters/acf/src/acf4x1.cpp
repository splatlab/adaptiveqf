...
//
	...
	int64_t tot_count=0;
	    ...
	    printf("TIME PER INSERT: %f\n", (double)(end_inserts - start_inserts) / j);
	    avg_insert_time += end_inserts - start_inserts;
	    avg_inserts += j;
		...
		A_ar.push_back(key);
		    ...
		    //unsigned int key = rand_zipfian(1.5f, 1lu << 30);
		    //fgets(buffer, sizeof(buffer), caida);
		    //csv_get(buffer, 3);
...
int main(int argc, char **argv) {
    ...
    HTmap<int64_t,int> cuckoo(4,1,atoi(argv[1]) / 4,1000);
    ...
    int num_trials = 5;
    for (trials = 0; trials < num_trials; trials++) run();

    FILE *progress = fopen("progress.csv", "w");
    fclose(progress);
    progress = fopen("progress.csv", "a");

    for (uint64_t i = 0; i < num_inserts / 100; i++) {
	   sprintf(buffer, "%lu,%f\n", i * 100, fprates[i]);
	   fputs(buffer, progress);
    }
