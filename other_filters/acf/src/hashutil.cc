...
uint64_t HashUtil::MurmurHash64(const void *key, size_t len, uint64_t seed) {
...
uint64_t HashUtil::MurmurHash64(const uint64_t key, uint64_t seed) {
	return MurmurHash64((void*)(&key), sizeof(uint64_t), seed);
}
