...
int main(int argc, char **argv)
{
	...
	uint64_t universe = -1;
	double load_factor = strtod(argv[3], NULL);
	uint64_t num_inserts = load_factor * nslots;
	...
	for (; trials < numtrials; trials++) {
		...
		uint64_t *inserted_set = calloc(num_inserts, sizeof(uint64_t));
		uint64_t inserted_set_len = 0;
		// PERFORM INSERTS
		...
		for (i = 0; qf.metadata->noccupied_slots < num_inserts; i++) {
			...
			if (!insert_key(&qf, htab, j, 1)) break;
			inserted_set[inserted_set_len++] = j;
		}
		...
		for (i = 0; i < num_queries; i++) {
			uint64_t elem_to_remove = inserted_set[rand_uniform(inserted_set_len)];
			//assert(qf_query(&qf, j, &ret_index, &ret_hash, &ret_hash_len, QF_KEY_IS_HASH));
			qf_remove(&qf, elem_to_remove, &ret_hash, &ret_hash_len, QF_KEY_IS_HASH | QF_NO_LOCK);
