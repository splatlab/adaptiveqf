...
int main(int argc, char **argv)
{
	...
	uint64_t next_measurement = current_measurement_point * qf.metadata->nslots;
	start_time = clock();
	for (i = 0; qf.metadata->noccupied_slots < target_fill; i++) {
		...
		if (qf.metadata->noccupied_slots > next_measurement) {
			printf("%f:\t%f\n", current_measurement_point, 1000000.0f * (measurement_interval * qf.metadata->nslots) / (clock() - start_time));
			current_measurement_point += measurement_interval;
			next_measurement = current_measurement_point * qf.metadata->nslots;
			start_time = clock();
		}
	}
	printf("%f:\t%f\n", current_measurement_point, 1000000.0f * (measurement_interval * qf.metadata->nslots) / (clock() - start_time));
