...
int main(int argc, char **argv)
{
	...
	sprintf(buffer, "queries,false positive rate,slots used\n");
	...
	double *slots_used_set = calloc(num_queries / step_size, sizeof(double));
    ...
    avgInsPer += (double)(clock() - start_time) / i;
		...
		if (0) while (i < num_queries) {
			...
			if (i % step_size == 0) {
				fpr = count_fp;
				fpr = (i == 0 ? 0 : fpr / i);
				fpr += fp_rate_set[i / step_size] * trials;
				fpr /= trials + 1;
				fp_rate_set[i / step_size] = fpr;

				slots_used_set[i / step_size] *= trials;
				slots_used_set[i / step_size] += qf_get_num_occupied_slots(&qf);
				slots_used_set[i / step_size] /= trials + 1;
	...
	for (uint64_t q = 0; q < num_queries / step_size; q++) {
		sprintf(buffer, "%lu,%f,%f\n", q * step_size, fp_rate_set[q], slots_used_set[q]);
		fputs(buffer, progress);
	}
	fclose(shalla);
	fclose(caida);
