...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--legacy") == 0) {
      ...
      fprintf(stderr, "Usage: %s [--legacy|--both] [--qbits N] [--num-queries N] [--load-factor F]\n", argv[0]);
      return 1;
    }
  }

  if (run_legacy) {
    if (run_aqf("legacy", true, qbits, num_queries, fill_factor)) return 1;
  }
  if (run_aqf_local) {
    if (run_aqf("aqf_local", false, qbits, num_queries, fill_factor)) return 1;
  }
  return 0;
}
